...
def sign(request: Request, username: str = Form(...), email: str = Form(...), role: str = Form("user"),
    ...
    try:
        existing_user = signup.find({"username": username})  # db query
        existing_email = signup.find({"email": email})  # db query
        if existing_user:
        ...
        if password != confirm:
         ...
         return html.TemplateResponse("signup.html", {"request": request, "error_message": http_error.detail})
    except Exception as e:
